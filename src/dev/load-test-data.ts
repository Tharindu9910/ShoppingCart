...
async function main() {
  ...
  await client.collection("shopping_list").deleteMany({});

  const resp = await client.collection("shopping_list").insertMany([
    {
      "id": "GGOEAFKA087499",
  ...
  console.info("Inserted items:", resp.insertedCount);
